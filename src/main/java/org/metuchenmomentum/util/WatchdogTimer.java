...
/**
 * A watchdog timer that runs events after a deadline
 * has expired. Timeouts can be canceled.
 *
 * <p>The implementation makes the following assumptions:
 *
 * <ul>
 *   <li>Some (unspecified) process invokes
 *       {@link WatchdogTimer#periodic periodic()}.</li>
 *       on every timer tick.
 *   <li>The inter-tick time is uniform.</li>
 *   <li>The robot will be rebooted before {@link Long#MAX_VALUE}
 *       {@code 2^63 -1}, roughly {@code 9.2233720368548E+18} ticks.
 *       If the tick has its default 20 ms setting, that's
 *       {@code 1.844674407×10¹⁷} seconds, which is a little
 *       mre than {@code 8093659099} years.</li>
 * </ul></p>
 *
 * <p>Note that this class is thread-safe.</p>
 */
public class WatchdogTimer {

  /**
   * Wraps around a {@link Runnable} to be invoked when its watchdog time
   * expires. Provides an absolute expiry time in ticks since construction,
   * a {@link Runnable} implementation that forwards to the user-provided
   * {@link Runnable}, and an {@link AutoCloseable} implementation that
  ...
  public static final class PendingAction
    ...
    /**
     * Active flag. If {@code true}, the expiry action will run. If
     * {@code false}, the action will not run.
     */
     ...
     *
     * @param onTimeout user-provided {@link Runnable} to be invoked
     *                  on expiry
     * @param deadlineInPeriodicTicks the absolute expiry time in
     *                                ticks since construction
     */
    private PendingAction(
          Runnable onTimeout,
          long deadlineInPeriodicTicks) {
    ...
    public int compareTo(PendingAction other) {
      return Long.compare(
          deadlineInPeriodicTicks,
          other.getDeadlineInPeriodicTicks());
    }

    /**
     * Closes this action, i.e. marks the enclose action as
     * bypassed. Note that this method is idempotent; multiple
     * invocations have the same effect as one.
     *
     * @see AutoCloseable
     */
    @Override
    ...
    /**
     *
     * @return the watchdog deadline in ticks since
     *         construction. Primarily used for testing.
     */
    ...
    /**
     * Runs the bound action if this {@link PendingAction}
     * has not been closed; does nothing otherwise.
     */
    @Override
    public void run() {
      if (active.get()) {
  ...
  /**
   * The number of ticks since this instance was constructed, with
   * each {@link #periodic()} invocation counting as a single
   * tick.
   */
  ...
  public void periodic() {
    ...
    try {
      while (
          !actions.isEmpty()
          && actions.peek().getDeadlineInPeriodicTicks() <= currentTime) {
        actions.take().run();
      }
  ...
  public AutoCloseable schedule(Runnable action, long delayInTicks) {
    var pendingAction = new PendingAction(
        action, ticksSinceConstruction.get() + delayInTicks);
  ...
  /**
   *
   * @return the number of pending actions. Note that the count
   *         <em>ALL</em> scheduled actions, including canceled
   *         actions.
   */
  public int size() {
    return actions.size();
