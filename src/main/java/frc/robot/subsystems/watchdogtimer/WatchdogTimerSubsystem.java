...
/**
 * Creates and drives a {@link WatchdogTimer}, a class that invokes invokes {@link Runnable}
 * implementations after a fixed delay. The subsystem passes timer ticks to the {@link
 * WatchdogTimer} so that it will invoke enqueued {@link Runnable} instances when their associated
 * timeouts expire.
 */
public final class WatchdogTimerSubsystem extends SubsystemBase {
  ...
  /** Constructor, instantiates a {@link WatchdogTimerSubsystem} with an empty queue. */
  public WatchdogTimerSubsystem() {
    watchdogTimer = new WatchdogTimer();
  }

  /** Advances the watchdog timer by 1 tick. Invoked during robot operation. */
  @Override
  public void periodic() {
    watchdogTimer.periodic();
  }

  /** Advances the watchdog timer by 1 tick. Invoked during robot simulation */
  @Override
