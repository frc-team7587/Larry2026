...
public final class MarqueeSubsystem extends SubsystemBase {
  ...
  /**
   * Convenience factory method: creates a {@link MarqueeSubsystem} communicates with the marquee
   * via a serial USB serial connection.
   *
  ...
  public static MarqueeSubsystem usbConnection(
      List<MarqueeMessage> messages, int millisecondsPerTick) {
    ...
    DataLogManager.log(
        "Created marquee connection type: " + marqueeConnection.getClass().getCanonicalName());
    return new MarqueeSubsystem(marqueeConnection, messages, millisecondsPerTick);
  }
