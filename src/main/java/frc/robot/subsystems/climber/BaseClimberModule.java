...
/**
 * Base climber module that implements the module life cycle and delegates low-level operatioons to
 * subclasses. There will be subclasses that drive both real (i.e. physical) and simulated climbers.
 */
public abstract class BaseClimberModule implements ClimberIO {

  private static final Consumer<BaseClimberModule> DO_NOTHING = (m) -> {};
  private static final Consumer<BaseClimberModule> STOP_WHEN_RAISED =
      (m) -> {
        m.stopWhenFullyExtended();
      };
  private static final Consumer<BaseClimberModule> STOP_WHEN_PARKED =
      (m) -> {
        m.stopWhenFullyRetracted();
      };
  ...
  /** Log an error message when the climber enters an unexpected or unrecognized state. */
   ...
   *
   * @return the motor position in revolutions relative to the motor's orientation at power-on.
   */
  protected abstract double doGetMotorPosition();
   ...
   */
  protected abstract void doSetMotorSpeed(double speed);
