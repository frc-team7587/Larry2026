 ...
 *
 * tODO: make this correspond with reality
 *
 * @see ClimberModule
 */
public class SimulatedClimberModule extends BaseClimberModule {
    ...
    /**
     * Updates the elevator simulation, then calculates the motor RPM from its
     * velocity and position. The spool has a .75" core and its flanges are
     * .562" apart. About 24" of .125" polycord are wond on the spool when the
     * climber is parked. If we model the wound cord as a solid cylender, we
     * can <em>roughly</em>estimage its outer diameter, which lets us estimate
     * (again roughly) the linear --> angular velocity conversion.
     */
    @Override
    public void simulationPeriodic() {
        ...
        var woundParacordLength =
            ClimberConstants.kParacordLengthMeters - extension;
        var totalVolume = ClimberConstants.kDrumVolumeCubitMeters
