...
/**
 * Climber configuration, including motion parameters and the climber life cycle. Note that the
 * climber is powered by a Neo with a 180:1 gear ratio.
 */
public class ClimberConstants {
  ...
  /**
   * The top of the hook is 20 1/16" +/=, above the ground, when fully retracted, approximately 51
   * cm.
   */
  static final double kRetractedElevation = .051;
  /**
   * The top of the hook is 29" +/- above the ground when fully extended, approximately 83.66 cm.
   */
  ...
  /**
   * The watchdog timeout in ticks, the robot's default interval between {@code
   * IteratedRobotBase.robotPeriodic()} invocations. The default value is 20 ms, 50
   * invocations/second. The timeout is arbitrarily set to 20 seconds.
   */
  ...
  /**
   * Estimated max motor speed. According to the <a
   * href=''https://www.revrobotics.com/content/docs/REV-21-1650-DS.pdf>NEO Brushless Motor Data
   * Sheet</a>, the maximum speed is just shy of 6000 RPM, but this produces 0 torque. Sticking a
   * finger in the air, we note that the torque peaks at 3000 ROM (+/->, so we use that for now.)
   */
  static final long kSimulatedMaxRPM = 3000;

  static final long kSimulatedMaxRevolutionsPerTick = (kSimulatedMaxRPM * 1000) / kTickTimeMillis;
}
