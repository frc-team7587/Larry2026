...
public class Drive extends SubsystemBase {
  ...
  // AllianceFlipUtil.apply(reefPoses[i]).minus(currentPose);
  ...
  /**
   * Gets the pose of the closest hub
   *
   * @param leftSide determines what side of the field your on
   */
  public Pose2d getClosestAprilTagOnHub(boolean leftSide) {
    Pose2d[] hubPoses = FieldConstants.Hub.AprilTagPoses;
    Pose2d currentPose = getPose();

    int closestPose = -1;
    double closestDistance = Double.MAX_VALUE;

    for (int i = 0; i < hubPoses.length; i++) {
      boolean isLeft = i % 2 == 0;
      if (leftSide != isLeft) {
        continue;
      }

      Transform2d currentToTarget = AllianceFlipUtil.apply(hubPoses[i]).minus(currentPose);
      double distance = currentToTarget.getTranslation().getNorm();
